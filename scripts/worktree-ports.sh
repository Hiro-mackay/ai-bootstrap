...
sanitize() {
  printf '%s' "$1" | tr '[:upper:]' '[:lower:]' | tr -c 'a-z0-9_-' '-' | sed 's/^-*//;s/-*$//'
}
...
cmd_alloc() {
  ...
  if is_main_worktree "$_dir"; then _off=0; else _off=$(pick_offset "$_dir") || exit 1; fi
  write_env_worktree "$_dir" "$_off"
  echo "worktree-ports: offset $_off -> pg $((BASE_PG + _off)) / api $((BASE_API + _off)) / web $((BASE_WEB + _off))"
}

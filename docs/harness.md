...
| `scripts/check-architecture.sh` -- layering, no-barrel, no cross-feature imports, file size, no domain serialization tags, no manual queryKey, no server state in stores, no escape hatches | Computational, architecture-specific. A custom sensor (with `check-context.sh` below) |
...
| `scripts/check-context.sh` -- context annotations: anchors carry `@context`/`@business`, every `@context` binds to `docs/domain-definitions.md` (the context-harness kernel, #34) | Computational. Vacuous until a context + domain code exist. Surface: `task context`. Grow via `docs/context-harness.md` |
